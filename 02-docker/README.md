...
この Step の目的: **コンテナのひな形（イメージ）とコンテナの違いを体験し、あとで Kubernetes が管理する「箱」を自分で作れるようにする。**
...
| 用語 | 意味 |
|------|------|
| イメージ | コンテナのひな形（設計図・テンプレ。例: `nginx:alpine`） |
...
**この節の目的:** イメージの管理（名前・版・中身の積み重ね）を見る。現場の「どの版を配置して動かしたか」問題の原型。
