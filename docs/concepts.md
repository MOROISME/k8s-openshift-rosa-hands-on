...
**クラスタ** = Kubernetes が管理する実行環境一式（コントロールプレーン + ノードの集まり）。

| 用語 | 意味 |
|------|------|
| クラスタ | K8s の管理単位となる全体 |
| ノード | コンテナ（Pod）が動くマシン |
| コントロールプレーン | クラスタの司令塔（API・スケジューリングなど） |
| Minikube | ローカル PC に学習用の小さなクラスタを作るツール |

本番は複数ノードが普通。Minikube は学習用に 1 ノード相当で、同じ `kubectl` 操作を練習できる。
